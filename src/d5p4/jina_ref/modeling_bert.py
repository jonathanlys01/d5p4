...
class JinaBertEncoder(nn.Module):
    ...
    def forward(
        ...
        if self._current_alibi_size < seqlen:
            ...
            self.register_buffer(
                "alibi",
                self.rebuild_alibi_tensor(size=seqlen, device=hidden_states.device).to(hidden_states.dtype),
                persistent=False,
            )
...
class JinaBertPreTrainedModel(PreTrainedModel):
    ...
    load_tf_weights = lambda *args, **kwargs: None  # disable TF weight load
...
class JinaBertForPreTraining(JinaBertPreTrainedModel):
    ...
    @add_start_docstrings_to_model_forward(BERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=JinaBertForPreTrainingOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
...
@add_start_docstrings("""JinaBert Model with a `language modeling` head on top.""", BERT_START_DOCSTRING)
