...
class ConditionCalculator:
  def __init__(self, tokenizer):
    import torch
    self.encoder_pipelines = {}  # Dictionary to store pipelines for different devices
    self.tokenizer = tokenizer

  def calculate_condition(self, batch):
    text_batch = self.convert_to_text(batch)
    batch_device = batch.device
    from sonar.inference_pipelines.text import TextToEmbeddingModelPipeline
    
    # Get or create pipeline for the batch's device
    if batch_device not in self.encoder_pipelines:
        self.encoder_pipelines[batch_device] = TextToEmbeddingModelPipeline(
            encoder="text_sonar_basic_encoder",
            tokenizer="text_sonar_basic_encoder",
            device=batch_device
        )
    
    # Use the appropriate pipeline for the batch's device
    embeddings = self.encoder_pipelines[batch_device].predict(text_batch, source_lang="eng_Latn")
    return embeddings.clone()

  def convert_to_text(self, batch):
    content = self.tokenizer.batch_decode(batch)
    return content