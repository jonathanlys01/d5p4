...
class Diffusion(L.LightningModule):
    ...
    def sample_subs_guidance(self, n_samples, stride_length, num_strides, dt=0.001):
        ...
        for i in range(2, len(intermediate_tokens) + 1):
            intermediate_text_samples.append(
                self.tokenizer.batch_decode(np.concatenate(intermediate_tokens[:i], axis=1))
            )
    ...
    def _compute_mask_conditional_entropy(self, x, t):
        ...
        sigma_t, _ = self.noise(t)
        model_output = self.forward(x, sigma_t)  # log probabilities

        # Convert to probabilities and ensure float32 for numerical stability
        probs = torch.softmax(model_output.float(), dim=-1)

        # Find masked positions
        masked_positions = x == self.mask_index  # shape: (batch_size, seq_len)

        # Compute Shannon entropy for each position
        # H = -sum(p * log2(p)) for each position
    ...
    def _sample_with_entropy(self, num_steps=None, eps=1e-5):
        """Generate samples from the model while tracking Denoising Entropy.

        Returns:
          samples: Generated samples
          entropy_trajectory: List of entropy values at each time step
          total_denoising_uncertainty: Integrated entropy over the process
        """
        batch_size_per_gpu = self.config.loader.eval_batch_size
        if self.parameterization == "ar":
            # For autoregressive models, we don't have the concept of masked diffusion
            samples = self._ar_sampler(batch_size_per_gpu)
            return samples, [], [0.0] * batch_size_per_gpu
        ...
        for i in range(num_steps):
            ...
            current_entropy = self._compute_mask_conditional_entropy(x, t)
            entropy_trajectory.append(current_entropy.float().cpu().numpy())
        ...
        if self.config.sampling.noise_removal:
            t = timesteps[-1] * torch.ones(x.shape[0], 1, device=self.device)
            final_entropy = self._compute_mask_conditional_entropy(x, t)
            entropy_trajectory.append(final_entropy.float().cpu().numpy())
        ...
        # Compute Total Denoising Uncertainty using trapezoidal integration
        # U_denoise = integral of H_MC(t) dt from 0 to 1
        total_denoising_uncertainty = []
        entropy_array = np.array(entropy_trajectory)  # shape: (num_steps, batch_size)

        for batch_idx in range(batch_size_per_gpu):
            batch_entropy_trajectory = entropy_array[:, batch_idx]
            # Use trapezoidal rule for numerical integration
            # We integrate from t=1 to t=eps, so we need to account for the time direction
            time_points = timesteps[: len(batch_entropy_trajectory)].cpu().numpy()
            integrated_entropy = np.trapz(batch_entropy_trajectory, x=time_points)
            total_denoising_uncertainty.append(
                abs(integrated_entropy)
            )  # Take absolute value since we integrate backwards

        return x, entropy_trajectory, total_denoising_uncertainty

    def restore_model_and_sample_with_entropy(self, num_steps, eps=1e-5):
        """Generate samples from the model while tracking Denoising Entropy."""
        if self.ema:
            self.ema.store(itertools.chain(self.backbone.parameters(), self.noise.parameters()))
            self.ema.copy_to(itertools.chain(self.backbone.parameters(), self.noise.parameters()))
        ...
        samples, entropy_trajectory, total_denoising_uncertainty = self._sample_with_entropy(
        ...
        return samples, entropy_trajectory, total_denoising_uncertainty
