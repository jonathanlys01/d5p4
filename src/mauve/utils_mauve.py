...
tqdm = lambda *args, **kwargs: tqdm_original(*args, **kwargs, disable=os.environ.get("DISABLE_TQDM", False))
...
def load_and_tokenize_json_data(tokenizer, data_path, max_len=1024, max_num_data=float("inf")):
    ...
    t1 = time.time()
    tokenized_texts = [tokenizer.encode(sen, return_tensors="pt", truncation=True, max_length=max_len) for sen in texts]
    t2 = time.time()
